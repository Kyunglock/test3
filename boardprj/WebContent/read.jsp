...
<%
	...
	int joinCount = board.getJoinCount();
	
	joinCount++;
	BoardDAO boardDAO = new BoardDAO();
	boardDAO.joinCount(itemNum, joinCount);
        	...
        	<div class="row">
        	    ...
        	    <table class="table table-striped style" style="text-align: center; border: 1px solid #dddddd">
        			<tbody>
        				<tr>
        					...
        					<td colspan ="2"><%=board.getItemTitle() %></td>
        				</tr>
        				<tr>        					
        					<td>작성자</td>
        				...
        				<tr>
        					<td>내용</td>
        					<td colspan ="2" style="min-height: 200px; text-align: left;"><%=board.getITEMCONTENT().replaceAll(" ", "&nbsp;").replaceAll("<", "$lt;").replaceAll(">", "&gt;").replaceAll("\n", "<br>") %></td>
        		...
        		%>
        				<div class="text-right">
        					<a href="update.jsp?itemNum=<%= itemNum %>" class="btn btn-primary">수정</a>
        					<a href="delete_proc.jsp?itemNum=<%= itemNum %>" class="btn btn-primary">삭제</a>
        				</div>
        		<%
        			}        		
        		 %>
        		 <form method="post" action="reply_proc.jsp?itemNum=<%= itemNum %>">
        		 <table style="text-align: center; border: 1px solid #dddddd" >
        		 	<tr height="50px">
        		 		<td width="10%" ><input type="submit" class="btn btn-primary" value= "댓글쓰기"></td>
        		 		<td width="2%"></td>
        		 		<td width="88%"><input type="text" class="form-control" placeholder="내용" name="replyContent" maxlength="50"></td>
        		 	</tr>
        		 </table>

        		</form>
        		<div id="flip">댓글보기</div>
        		<div id="panel">
				    <table id="panel" class="table table-striped" style="margin:auto; text-align:center;">
	
	              		<tbody>
	          		  		<%  	
					  			ReplyDAO replyDAO = new ReplyDAO();
		          		 	 	ArrayList<Reply> list;
		          		  		int listNum = Integer.parseInt(request.getParameter("itemNum"));
		          		  		list = replyDAO.getList(listNum);
						  		for(int i=0; i< list.size(); i++){
					  		%>
	                	<tr>
		                	<td width = "20%">
		                		<%=list.get(i).getReplyUserID() %>:
		                	</td>
