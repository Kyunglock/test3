...
<%
	...
	if( userID == null) {
		...
		if( board.getItemTitle() == null || board.getITEMCONTENT() ==null){
			PrintWriter script = response.getWriter();
			script.println("<script>");
			...
			int result = boardDAO.reUpdate(itemNum, request.getParameter("ITEMCONTENT"), request.getParameter("itemTitle"));
			if( result == -1){
				PrintWriter script = response.getWriter();
				script.println("<script>");
